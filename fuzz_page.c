 ...
 *
 * Agent-written scaffolding (crucible rule): this file is the contract and
 * the checker. The implementations belong to the human, in mosken.c.
 *
 * CONTRACT — implement these three, then move the declarations into
 * mosken.h:
 *
 *   void page_delete_item(Page page, OffsetNum offset_number);
 *       Tombstone the slot. The item's bytes stay in place; lower and
 *       upper do not move. Slot-state encoding is your design decision.
 ...
 *       items and requires them to stay live; if you pick reject-at-add
 *       instead, say so and the harness stops generating empties (my edit).
 *
 ...
 *       1 if the slot is live, 0 if tombstoned. (Hides your encoding.)
 *
 ...
 * you loosen it to `>=`, adjust WOULD_FIT below.
 *
...
static uint32_t
model_lower(const struct model *m)
{
...
static void
gen_item(unsigned char *buf, uint32_t *len_out)
{
...
static void
verify(Page page, const struct model *m)
{
...
static void
fuzz_seed(uint64_t seed, uint32_t ops)
{
